...
namespace VeilingklokUnitTest.Feature.Auth
{
    public sealed class JwtServiceTests
    {
        ...
        public void GenerateToken_IncludesRoleEmailNameIdClaims()
        {
            ...
            static string? GetClaim(JwtSecurityToken token, string type) =>
                token.Claims.FirstOrDefault(c => c.Type == type)?.Value;

            // Check dat alle verwachte claims aanwezig en correct zijn
            Assert.Equal(gebruiker.Id.ToString(), GetClaim(jwt, ClaimTypes.NameIdentifier));
            Assert.Equal(gebruiker.Email, GetClaim(jwt, ClaimTypes.Email));
            Assert.Equal(gebruiker.Rol.ToString(), GetClaim(jwt, ClaimTypes.Role));
            Assert.Equal(gebruiker.Voornaam, GetClaim(jwt, ClaimTypes.GivenName));
            Assert.Equal(gebruiker.Achternaam, GetClaim(jwt, ClaimTypes.Surname));
            Assert.Equal($"{gebruiker.Voornaam} {gebruiker.Achternaam}", GetClaim(jwt, ClaimTypes.Name));
        }
